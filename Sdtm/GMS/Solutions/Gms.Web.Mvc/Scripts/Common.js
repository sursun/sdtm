...
$(function () {
    ...
    $('a.export').live('click', function () {
        var self = $(this);
        var url = this.href + '?' + $.param(self.parents('.search_region').find(':input').SerializeElements());
        window.open(url);
        return false;
    });
    ...
    $('.select_show_view').live("change", function () {
        var vid = $(this).attr('forview');
        var curVal = $(this).val();
        var selVal = $(this).attr('selectval');
        var divView = $('#' + vid);

        if (curVal == selVal) {
            divView.show();
            var url = $("iframe", divView).attr("src");
            if (url != undefined) {
                $("iframe", divView).attr("src", url);
            }
